...
```
從物件中提出的`id`, `displayNmae` 和`firstName` 並印出
...
```
sumValue的parameter為物件，想像
```
inputArray = {array1: array1, array2: array2}
```
因此在第3行中
`const length = inputArray.arr1.length;`，會以物件`.`的方式取`arr1`
